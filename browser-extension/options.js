...
async function load() {
  const stored = await chrome.storage.local.get(['relayPort', 'gatewayToken', 'autoReconnect'])
  const port = clampPort(stored.relayPort)
  const token = String(stored.gatewayToken || '').trim()
  const autoReconnect = stored.autoReconnect === true
...
async function saveAutoReconnect() {
  ...
  await chrome.storage.local.set({ autoReconnect })
  console.log('Auto-reconnect setting saved:', autoReconnect)
}

document.getElementById('save').addEventListener('click', () => void save())
document.getElementById('autoReconnect').addEventListener('change', () => void saveAutoReconnect())
void load()
