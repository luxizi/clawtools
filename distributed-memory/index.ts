...
import { classifyWithLLM, type LLMClassificationConfig } from "./src/llm-classifier.js";
...
const memoryLanceDBProPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Uses ASYNC fire-and-forget pattern: classification runs in background,
    // does not block the agent response. Flow: small model first → rules fallback.
    if (config.autoCapture !== false) {

      /**
       * Extract user (and optionally assistant) texts from event messages.
       */
      function extractTexts(messages: unknown[], captureAssistant: boolean): string[] {
        const texts: string[] = [];
          ...
          if (typeof content === "string") {
            ...
            for (const block of content) {
              if (
                ...
                "type" in block && (block as Record<string, unknown>).type === "text" &&
      ...
      /**
       * Async classification + storage pipeline (fire-and-forget).
       * Flow: LLM small model → fallback to rules → embed & dedupe → store.
       */
      async function asyncCaptureAndStore(
        texts: string[],
        defaultScope: string,
        agentId: string,
      ): Promise<void> {
        const toCapture: Array<{ text: string; category?: string; reason?: string }> = [];

        for (const text of texts) {
          if (!text) continue;

          // ── Step 1: Try LLM small model classification ──
          if (config.llmClassification?.enabled) {
            try {
              const llmResult = await classifyWithLLM(text, config.llmClassification);
              if (llmResult) {
                api.logger.info(
                  `memory-lancedb-pro: [async] LLM classification: worth=${llmResult.worthRemembering}, ` +
                  `category=${llmResult.category}, reason="${llmResult.reason}"`
                );
                if (llmResult.worthRemembering) {
                  toCapture.push({ text, category: llmResult.category, reason: llmResult.reason });
                }
                continue; // LLM succeeded, skip rules
              }
              // LLM returned null → fall through to rules
              api.logger.info(`memory-lancedb-pro: [async] LLM returned null for "${text.slice(0, 40)}…", falling back to rules`);
            } catch (err) {
              api.logger.warn(`memory-lancedb-pro: [async] LLM error: ${String(err)}, falling back to rules`);
            }
          }
        ...
        if (toCapture.length === 0) {
          api.logger.info(`memory-lancedb-pro: [async] no capturable content`);
          return;
        }

        // ── Step 3: Embed, dedupe, store (limit 3 per conversation) ──
        let stored = 0;
        for (const item of toCapture.slice(0, 3)) {
          const { text, category: llmCategory, reason } = item;
          const category = llmCategory || detectCategory(text);

          const vector = await embedder.embedPassage(text);

          // Dedupe: skip if very similar entry already exists
          const existing = await store.vectorSearch(vector, 1, 0.1, [defaultScope]);
          if (existing.length > 0 && existing[0].score > 0.95) {
            api.logger.info(`memory-lancedb-pro: [async] skipping duplicate: "${text.slice(0, 40)}…"`);
            continue;
          }
          ...
          api.logger.info(
            `memory-lancedb-pro: [async] stored "${text.slice(0, 40)}…" as ${category} in ${defaultScope}` +
      ...
      // ── Hook: agent_end → fire-and-forget async capture ──
      api.on("agent_end", async (event) => {
        ...
        const texts = extractTexts(event.messages, config.captureAssistant === true);

        if (texts.length === 0) return;

        api.logger.info(
          `memory-lancedb-pro: [async] firing background capture for ${texts.length} texts (agent: ${agentId})`
        );

        // Fire-and-forget: do NOT await — let the response return immediately.
        asyncCaptureAndStore(texts, defaultScope, agentId).catch((err) => {
          api.logger.warn(`memory-lancedb-pro: [async] background capture failed: ${String(err)}`);
        });
      });
    }
...
function parsePluginConfig(value: unknown): PluginConfig {
    ...
    return {
      ...
      sessionMemory: typeof cfg.sessionMemory === "object" && cfg.sessionMemory !== null
        ...
        : undefined,
      llmClassification: typeof cfg.llmClassification === "object" && cfg.llmClassification !== null
        ? (() => {
            const lc = cfg.llmClassification as Record<string, unknown>;
            return {
              enabled: lc.enabled === true,
              provider: (lc.provider as "kimi" | "ollama") || "ollama",
              model: typeof lc.model === "string" ? lc.model : undefined,
              apiKey: typeof lc.apiKey === "string" ? resolveEnvVars(lc.apiKey) : undefined,
              baseURL: typeof lc.baseURL === "string" ? resolveEnvVars(lc.baseURL) : undefined,
              maxRetries: typeof lc.maxRetries === "number" ? lc.maxRetries : 1,
              timeout: typeof lc.timeout === "number" ? lc.timeout : 30000,
              fallbackToRules: lc.fallbackToRules !== false, // default true
            } as LLMClassificationConfig;
          })()
        : undefined,
    };
}
